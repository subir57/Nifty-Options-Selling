...
def _nse_get_json(url, params=None, retries=4):
    """GET an NSE JSON endpoint without letting an HTML/CSV response crash Streamlit.

    NSE can return an anti-bot HTML page, a temporary gateway response, or (for
    some historical endpoints) CSV depending on query parameters.  The caller
    needs structured JSON, so validate the response before decoding it and
    refresh the NSE session between retries.
    """
    last = None
    for attempt in range(retries):
        try:
            session = nse_http_session()
            r = session.get(url, params=params, timeout=30)
            text = r.text or ""
            ctype = (r.headers.get("Content-Type") or "").lower()

            if r.status_code == 200:
                # Avoid the unhelpful JSONDecodeError that previously killed the
                # Streamlit app when NSE returned CSV/HTML instead of JSON.
                if "json" in ctype:
                    try:
                        return r.json()
                    except ValueError as exc:
                        last = RuntimeError(
                            f"NSE returned invalid JSON from {url}. "
                    ...
                    last = RuntimeError(
                        f"NSE returned non-JSON data (HTTP 200, {ctype or 'unknown content type'}): {preview}"
                    )
            else:
                last = RuntimeError(f"NSE HTTP {r.status_code}: {text[:300]}")

            # Refresh the cookie/session after blocked, malformed or throttled
            # responses.  This is especially important on Streamlit Cloud where
            # the outbound IP can be shared.
            if r.status_code in (401, 403, 429) or last is not None:
                try:
                    session.get(f"{NSE_BASE}/", timeout=20)
                    session.get(f"{NSE_BASE}/report-detail/fo_eq_security", timeout=20)
                except Exception:
                    pass
...
def nse_equity_history(symbol, start_date, end_date):
    ...
    params = {
        ...
        # Request JSON. The NSE endpoint returns CSV when csv=true; the
        # backtest engine expects the structured JSON response.
        "series": "EQ",
        "csv": "false",
    }
